...
const AccountCreationModalBody = ({
	...
	const availableOrganizations = useMemo(() => {
		...
		return resource.filter((org: any) => !accountValues.includes(org.value));
					...
					{accountTypes.map((type: any) => (
						<ClaySelect.Option
							...
							label={`${type[0].toUpperCase()}${type
								.substr(1)
								.toLowerCase()}`}
							onChange={(event: React.FormEvent<HTMLOptionElement>) =>
								setAccountData({
									...accountData,
									type: (event.target as HTMLOptionElement).value,
								})
							}
