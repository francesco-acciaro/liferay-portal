...
function clone_repository {
	...
	rm --force --recursive "${LIFERAY_LEARN_ETC_CRON_IMPORTER_GIT_REPOSITORY_DIR}"

	git clone \
		--branch "${LIFERAY_LEARN_ETC_CRON_IMPORTER_GITHUB_BRANCH:-master}" \
		--depth 1 \
		--single-branch \
		git@github.com:"${LIFERAY_LEARN_ETC_CRON_IMPORTER_GITHUB_USER:-liferay}"/liferay-learn.git \
		"${LIFERAY_LEARN_ETC_CRON_IMPORTER_GIT_REPOSITORY_DIR}"

	git \
		-C "${LIFERAY_LEARN_ETC_CRON_IMPORTER_GIT_REPOSITORY_DIR}" \
		log \
		--max-count=1 \
...
function write_manifest {
	...
	zips_json=$( \
		cd "${LIFERAY_LEARN_ETC_CRON_IMPORTER_GIT_REPOSITORY_DIR}/site" && \
